...
namespace my_stl
{
	...
	//multimap定义
	template<typename Key, typename T, typename Compare = std::less<Key>, typename Allocator = my_stl::allocator<pair<const Key, T> > >
	class multimap
	{
	public:
		typedef multimap<Key, T, Compare, Allocator> Myt;
		typedef Key key_type;
		typedef T mapped_type;
		typedef pair<const Key, T> value_type;
		typedef size_t size_type;
		...
		//友元声明
		template< typename Key, typename T, typename Compare, typename Alloc >
		friend bool operator==(const multimap<Key, T, Compare, Alloc>& lhs, const multimap<Key, T, Compare, Alloc>& rhs);
		template< typename Key, typename T, typename Compare, typename Alloc >
		friend bool operator<(const multimap<Key, T, Compare, Alloc>& lhs, const multimap<Key, T, Compare, Alloc>& rhs);
		...
		explicit multimap(const Compare &comp = Compare(), const Allocator &alloc = Allocator()) : t(comp, alloc) {}
		explicit multimap(const Allocator& alloc) : t(alloc) {}
		
		template< class InputIterator >
		multimap(InputIterator first, InputIterator last,
			const Compare& comp = Compare(),
			const Allocator& alloc = Allocator())
			: t(comp, alloc) { t.insert_equal(first, last); }
		multimap(const multimap& other) : t(other.t) {}
		multimap(const multimap &ither, const Allocator &alloc) : t(other.t, alloc) {}
		multimap(multimap&& other) : t(std::move(other.t)) {}
		multimap(multimap&& other, const Allocator& alloc) : t(std::move(other.t), alloc) {}
		multimap(std::initializer_list<value_type> init,
			const Compare& comp = Compare(),
			const Allocator& alloc = Allocator())
			: t(comp, alloc) { t.insert_equal(init.begin(), init.end()); }

		multimap& operator=(const multimap& other){ t = other.t; return *this; }
		multimap& operator=(multimap&& other) { t = std::move(other.t); return *this; }
		multimap& operator=(std::initializer_list<value_type> ilist) { t = ilist; return *this; }
		allocator_type get_allocator() const { return t.get_allocator(); }
		...
		template< class InputIt >
		void insert(InputIt first, InputIt last) { t.insert_equal(first, last); }
		void insert(std::initializer_list<value_type> ilist) { t.insert_equal(ilist.begin(), ilist.end()); }
		...
		iterator erase(const_iterator first, const_iterator last) { return t.erase(first, last); }
		size_type erase(const key_type& key) { return t.erase(key); }
		void swap(multimap& other) { t.swap(other.t); }
