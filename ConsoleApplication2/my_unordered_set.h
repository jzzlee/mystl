...
namespace my_stl
{
	...
	{
	public:
		typedef unordered_multiset<Key, Hash, KeyEqual, Allocator> Myt;
		typedef Key key_type;
		typedef Key value_type;
		typedef size_t size_type;
		typedef ptrdiff_t difference_type;
		...
		template< class Key, class Hash, class KeyEqual, class Allocator >
		friend bool operator==(const unordered_multiset<Key, Hash, KeyEqual, Allocator>& lhs,
			const unordered_multiset<Key, Hash, KeyEqual, Allocator>& rhs);

		template< class Key, class Hash, class KeyEqual, class Alloc >
		friend void swap(unordered_multiset<Key, Hash, KeyEqual, Alloc>& lhs,
			unordered_multiset<Key, Hash, KeyEqual, Alloc>& rhs);

		//大批构造函数
		explicit unordered_multiset(size_type bucket_count = __bucket_init_count,
			const Hash& hash = Hash(),
			const KeyEqual& equal = KeyEqual(),
			const Allocator& alloc = Allocator())
			: table(bucket_count, hash, identity_type(), equal, alloc) {}

		explicit unordered_multiset(const Allocator& alloc) : table(alloc) {}
		...
		unordered_multiset(const unordered_multiset& other) : table(other.table) {}
		unordered_multiset(const unordered_multiset& other, const Allocator& alloc) : table(other.table, alloc) {}
		unordered_multiset(unordered_multiset&& other) : table(std::move(other.table)) {}
		unordered_multiset(unordered_multiset&& other, const Allocator& alloc) : table(std::move(other.table), alloc) {}
